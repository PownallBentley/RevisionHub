...
import { faBookOpen, faChevronDown, faSignOutAlt, faCog, faUser } from "@fortawesome/free-solid-svg-icons";
...
export default function AppHeader() {
  ...
  const { user, profile, isChild, activeChildId, signOut } = useAuth();
    ...
    <header className="bg-white border-b border-gray-100 sticky top-0 z-50">
      <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
        ...
        <Link to="/" className="flex items-center gap-3 text-gray-900 hover:opacity-80 transition-opacity">
          <div className="w-10 h-10 bg-brand-purple rounded-xl flex items-center justify-center shadow-sm">
          ...
          <div>
            ...
            <div className="text-xs text-gray-500">Parent-led revision</div>
          </div>
        </Link>
        ...
        {!isLoggedIn ? (
          ...
          <div className="relative" ref={dropdownRef}>
            ...
            >
              {/* Avatar */}
              <div className="w-9 h-9 bg-brand-purple rounded-full flex items-center justify-center text-white text-sm font-semibold">
                {initials}
              </div>
              {/* Name */}
                ...
                {!isChild && (
                  <>
                    <button
                      ...
                      className="w-full px-4 py-2.5 text-left text-sm text-gray-700 hover:bg-gray-50 flex items-center gap-3 opacity-50 cursor-not-allowed"
                      disabled
                    >
